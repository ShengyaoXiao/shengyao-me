...
const Navbar = ({ items, currentSection, currentScroll, onItemClick }) => {
  ...
  return (
    <div>
      <div
        className={"navbar navbar-dimensions text-unselectable " + className}
      >
        {items.map((item) => {
          return (
            // item.name === "blog"?
            ...
            <span
              ...
              className={
                "navbar-item navbar-item-dimensions clickable" +
                (item.name === currentSection ? " navbar-item-selected" : "")
              }
            >
              {item.name}
            </span>
